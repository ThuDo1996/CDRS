...
class Data(object):
    def __init__(self, path, domain, args):
        ...
        self.full_data = pd.concat([self.train, valid, test])

        ### Data statistic
        self.args = args
        self.n_users = max(self.full_data["uid"]) + 1
        self.n_items = max(self.full_data['iid']) + 1
        ...
        self.valid = self.create_test_ranking(self.full_data, valid)
        self.test = self.create_test_ranking(self.full_data, test)    
